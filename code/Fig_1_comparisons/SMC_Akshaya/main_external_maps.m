...
for maps = 5%:654
    ...
    try
        mapTemp = py.numpy.load(strcat('prob_map_npys/',num2str(maps),'.npy'));
    catch
        continue;
    end
    informationMap = double(mapTemp);
    Z = zeros(size(X));
    ...
    % mat_size = size(X); 
    % eee_mat = zeros(mat_size);
    % for i = 1:mat_size(1)
    %     for j = 1:mat_size(2)
    %         eee_mat(i,j) = norm([i j]-[pose.x pose.y])*info_map(i,j);
    %     end
    % end
    % eee_val = sum(sum(eee_mat));
    ...
    % drawnow
    simtime(maps) = toc;
    % saveas(gcf,'end SMC 1,2.fig')
    
    % fprintf('EEE Value: %f\n', eee_val);
    fprintf('Ergodicity Value: %d\n', Ergodicity_Metric(end));
    
    ergodicities(maps) = Ergodicity_Metric(end); 
    


end

save("times_ergs.mat", "ergodicities", "simtime"); 
...
% figure; plot(time(1:end),Ergodicity_Metric(1:end))
